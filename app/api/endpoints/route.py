...
from app.crud.route import create_route, get_routes, get_route_by_id, update_route, delete_route
...
def get_route_by_id_endpoint(route_id: int, db: Session = Depends(get_db),
                             current_user: TokenData = Depends(get_current_user)
                             ):
    """
      Get Route by ID / Obtener ruta por ID

      English:
      --------
      Retrieve a cycling route by its ID.

      - **route_id** (required): ID of the route to retrieve.

      Español:
      --------
      Recuperar una ruta de ciclismo por su ID.

      - **route_id** (requerido): ID de la ruta a obtener.
      """
    if current_user.role.value not in [Role.ADMIN]:
        raise HTTPException(status_code=403, detail="Not enough permissions")
